...
class Cache {
	...
	public function init() {
		...
		if ( $this->is_object_cache_enabled() && $this->is_page_cache_enabled() ) {
			$this->admin_bar->add_item( __( 'Purge All Caches', 'wewp' ), 'purge-all' );
		}

		if ( $this->is_object_cache_enabled() ) {
			$this->admin_bar->add_item( __( 'Purge Object Cache', 'wewp' ), 'purge-object' );
		}

		if ( $this->is_page_cache_enabled() ) {
			$this->admin_bar->add_item( __( 'Purge Page Cache', 'wewp' ), 'purge-page' );
			$this->cli->register_command( 'WeWP cache', CacheCommands::class );
		}

		if ( $this->is_page_cache_enabled() && ! is_admin() ) {
			$this->admin_bar->add_item( __( 'Purge This URL', 'wewp' ), 'purge-url' );
		}

		add_action( 'wewp_purge_object_cache', array( $this, 'purge_object_cache' ) );
		add_action( 'wewp_purge_page_cache', array( $this, 'purge_page_cache' ) );
		add_action( 'wewp_purge_url', array( $this, 'purge_url' ) );
		...
		add_action( 'init', array( $this, 'wewp_admin_notices' ), 999 );
	}
	...
	public function handle_manual_purge_action() {
		...
		$action = filter_input( INPUT_GET, 'wewp_action' );
		...
		// Set transient for the admin notice
	    set_transient('wewp_cache_clear_notice', array(
	        'success' => $purge,
	        'cache_type' => $type
	    ), 300);
	...
	public function wewp_admin_notices() {
		
		$notice = get_transient('wewp_cache_clear_notice'); 
	    if ( !empty( $notice ) && is_admin() ) {

	        $type = $notice['cache_type'];
        	...
        	delete_transient('wewp_cache_clear_notice');
		}
	}
	...
	public function purge_post_on_delete( $post_id ) {
		...
		return $this->purge_post( get_post($post_id) );
	}
	...
	private function set_cache_path() {
		if ( getenv( 'WEWP_CACHE_PATH' ) ) {
			$this->cache_path = getenv( 'WEWP_CACHE_PATH' );
		}

		if ( defined( 'WEWP_CACHE_PATH' ) ) {
			$this->cache_path = WEWP_CACHE_PATH;
		}
	}
	...
	public function is_page_cache_enabled() {
		return defined( 'WEWP_CACHE_PATH' ) || getenv( 'WEWP_CACHE_PATH' );
	}
	...
	public function purge_page_cache() {
		...
		do_action( 'wewp_site_purged', $result );
	...
	public function purge_post( $post ) {
		...
		do_action( 'wewp_post_purged', $post, $result );
	...
	public function purge_url( $url ) {
		...
		foreach ($cache_paths as $path) {
			...
			do_action( 'wewp_url_purged', $url, $deleted );
			$all_deleted &= $deleted;
		}
	 ...
	 * but it can be modified with wewp_cache_key_components filter.
	...
	private function get_cache_keys_for_url( $url ) {
		...
		$cache_keys = apply_filters('wewp_cache_keys_for_url', $cache_keys, $url);
	...
	private function delete( $path, $recursive = false ) {
		...
		if ( null !== $path && is_dir( $path ) && is_writable( $path ) ) {
	...
	/**
	 * Use wewp daemon to purge cache.
	 *
	 * @param string $path
	 *
	 * @return bool
	...
	private function should_purge_post_status( $new_status, $old_status ) {
		...
		return apply_filters( 'wewp_should_purge_post_status', true );
	}
	...
	private function get_public_post_types() {
		...
		return apply_filters( 'wewp_public_post_types', $post_types );
	}
	...
	private function get_post_types_needing_single_purge() {
		return apply_filters( 'wewp_post_types_needing_single_purge', array() );
	}
	...
	private function get_post_types_excluded_from_purge() {
		return apply_filters( 'wewp_post_types_excluded_from_purge', array(
			'attachment',
			'custom_css',
			'revision',
			'user_request'
